...
<PresentifyProvider options={{ theme: localStorage.getItem("joy-mode"), className: "presentation" }}>
    ...
    const FetchPosts = () => {
        ...
        if(status === "success"){  // or isSuccess
            return <div>Loading...</div> // query was successful and data is available
        }
    }
    ```
    ...
    const FetchPost = ({id}) => {
        const { data: {post}, isPending } = useQuery({queryKey: ["posts", id], queryFn: () => fetchPost(id)});
    ...
    const FetchPosts = () => {
        const { data: posts, isPending, isError } = useQuery({queryKey: ["posts"], queryFn: fetchPosts})
    ...
    const FetchPosts = () => {
        const { data: posts, isSuccess } = useQuery({queryKey: ["posts"], queryFn: fetchPosts})
    ...
    ---
        ...
        const AddPost = () => {
            ...
            const { addPost } = useMutation({
                ...
                onSuccess: () => {
                    queryCache.invalidateQueries({queryKey: ["posts"]});
                }
            });
