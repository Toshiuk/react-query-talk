...
<PresentifyProvider options={{ theme: localStorage.getItem("joy-mode"), className: "presentation" }}>
    ...
    const FetchPosts = () => {
        const { data: posts, isLoading, isError } = useQuery("posts", fetchPosts, onError: (error) => {
    ...
    const FetchPosts = () => {
        const { data: posts, isSuccess } = useQuery("posts", fetchPosts, onSuccess: () => {
            showSnackbar("Posts loaded sucessufully!");
        })
